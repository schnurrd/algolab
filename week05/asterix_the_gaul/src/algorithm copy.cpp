...
void testcase() {
	...
	for (int s = 0; s < (n_half + 1); s++){
		std::sort(movements_time_and_num_movements[s].begin(), 
				movements_time_and_num_movements[s].end(), 
			[](std::pair<long, long> a, std::pair<long, long> b) { return a.first < b.first or (a.first == b.first and a.second > b.second);});
		int dist = -1;
		int shift = 0;
		for (int i = 0; i < (int) movements_time_and_num_movements[s].size(); i++){
			if (movements_time_and_num_movements[s][i].second > dist){
				dist = movements_time_and_num_movements[s][i].second;
				movements_time_and_num_movements[s][shift] =  movements_time_and_num_movements[s][i];
				shift++;
			} 
		}
		movements_time_and_num_movements[s].resize(shift);
	}
	for (int s = 0; s < (n - n_half + 1); s++){
		std::sort(movements_time_and_num_movements_2[s].begin(), movements_time_and_num_movements_2[s].end(), 
			[](std::pair<long, long> a, std::pair<long, long> b) { return a.first < b.first or (a.first == b.first and a.second > b.second);});
		int dist = -1;
		int shift = 0;
		for (int i = 0; i < (int) movements_time_and_num_movements_2[s].size(); i++){
			if (movements_time_and_num_movements_2[s][i].second > dist){
				dist = movements_time_and_num_movements_2[s][i].second;
				movements_time_and_num_movements_2[s][shift] =  movements_time_and_num_movements_2[s][i];
				shift++;
			} 
		}
		movements_time_and_num_movements_2[s].resize(shift);
	}
